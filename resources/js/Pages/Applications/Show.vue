<template>
	<main-layout>
		<div class="flex justify-between items-center">
			...
			<template v-if="currentUser.userable_type === 'company' || currentUser.is_admin">
			...
			<div v-else-if="currentUser.userable_type === 'student' && application.status === null" class="flex">
			...
			<card class="col-span-8" title="Applicant Information" subtitle="Personal details and application.">
				<div class="">
					<dl>
						...
						<div v-if="application.attachments && application.attachments.length" class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 border-t border-gray-200">
							<dt class="text-sm font-medium text-gray-500">Attachments</dt>
							<dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
								<ul class="border border-gray-200 rounded-md divide-y divide-gray-200">
									<li v-for="attachment in application.attachments" :key="attachment.url || attachment.name" class="pl-3 pr-4 py-3 flex items-center justify-between text-sm">
										<div class="w-0 flex-1 flex items-center"><svg class="flex-shrink-0 h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M8 4a3 3 0 00-3 3v4a5 5 0 0010 0V7a1 1 0 112 0v4a7 7 0 11-14 0V7a5 5 0 0110 0v4a3 3 0 11-6 0V7a1 1 0 012 0v4a1 1 0 102 0V7a3 3 0 00-3-3z" clip-rule="evenodd" /></svg><span class="ml-2 flex-1 w-0 truncate">{{ attachment.name }}</span></div>
										<div class="ml-4 flex-shrink-0"><a :href="attachment.url" target="_blank" class="font-medium text-blue-600 hover:text-blue-500">Download</a></div>
									</li>
								</ul>
							</dd>
						</div>
